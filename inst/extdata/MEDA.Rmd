...
```{r meda-mclust}
tryCatch(mc1 <- p.mclust(out$data, out$bic, maxd = 8, print = ifelse(dim(dat)[1]<100,TRUE,FALSE)))
```

## Binary Hierarhical Mclust classifications
```{r meda-hmclust}
tryCatch(hmcL <- p.hmclust(dat, truth = truth, maxd = 8))
```
...
```{r meda-ccov, fig.height = 8 +ifelse(dim(hmcL[[2]])[3] %% 2==0,dim(hmcL[[2]])[3]- 2,dim(hmcL[[2]])[3])}
...
```{r meda-rsvFULL, fig.show = 'hide', fig.keep='all', dev = 'pdf', }
tryCatch(rsvFULL <- p.rsv(dat, ccol = colCol, maxd = Inf))
rsvFULL[[1]]
rsvFULL[[2]]
```

The full RSV plots can be downloaded [here](./meda-rsvFULL-1.pdf)
and [here](./meda-rsvFULL-2.pdf).
