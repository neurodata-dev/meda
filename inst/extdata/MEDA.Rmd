...
tryCatch(hmcL <- p.hmc(dat, truth = truth, maxDim = 8))
```
...
tryCatch(param <- p.clusterMeans(hmcL$means, ccol = colCol))
...
The default maxiumum tree depth is set to 6.
```{r meda-dendro}
plot(as.dendrogram(hmcL), center = TRUE)
round(hmcL$Get("num", filterFun=isLeaf), 4)
```
