...
d1heat <- function(dat, breaks = "Scott", ccol = "black") {
  ...
  x <- data.table(apply(dat, 2, as.numeric))

  ### Replace outer 1% of points with NAs
  #qs <- apply(x, 2, quantile, probs = c(0.01, 0.99))
  x <- x[, lapply(.SD, function(x) {
            qs <- quantile(x, probs = c(0.05, 0.95))
            x[x < qs[1]] <- NA
            x[x > qs[2]] <- NA
            return(x)
        })]
  ...
  mt <- data.table::melt(x, measure = 1:ncol(dat))

  H <- hist(mt$value, breaks = breaks, plot = FALSE)
  df <- expand.grid(x = H$mids, y = names(dat))

  bn <- lapply(apply(x, 2, hist, breaks = H$breaks, plot = FALSE), 
               function(x) { x$counts }) 
...
plot.d1heat <- function(x, ...){
  ...
  mycol <- if(!is.null(list(...)$col)){
             list(...)$col
           } else {
             colorpanel(255, "white", "#094620")
           }
